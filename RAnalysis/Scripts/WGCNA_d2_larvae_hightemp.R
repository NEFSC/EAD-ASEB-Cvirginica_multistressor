...
d2.filtered_count_tble  <- read.csv(file="Data/TagSeq/Filtered_counts/filtered_counts_5cpm_50perc/day2.filtered_5cpm50perc_hightemp.csv", sep=',', header=TRUE) 
...
png("Output/WGCNA/day2_larvae_hightemp/Day2_ClusterTree_Precut.png", 1000, 1000, pointsize=20)
plot(sampleTree, main = "Sample clustering to detect outliers", sub="", xlab="", cex.lab = 1.5, 
     cex.axis = 1.5, cex.main = 2) # appears there are two outliers SG59; can remove by hand or an automatic appraoch 
abline(h = 40, col = "red") # add line to plot to show the cut-off od outlier samples (40000) SG105 and SG55
dev.off()
...
clust = cutreeStatic(sampleTree, cutHeight = 40, minSize = 10) # Determine cluster under the line
...
png("Output/WGCNA/day2_larvae_hightemp/Day2_ClusterTree_Postcut.png", 1000, 1000, pointsize=20)
...
dim(dds.d2_vst) # 11 4858- transformed count data now  has 35 samples - 1 cut in the tree step above 
dim(d2.Treatment.data) # 12  7 - trait data has  36 samples - not yet cut! 
...
dim(d2.Traits) # 11 Samples 6 columns; now we have 22 samples! - colnames are all treatment, primary and second treatment
...
# pCO2 ONLY
png("Output/WGCNA/day2_larvae_hightemp/Day2_ClusterTree_pCO2.png", 1000, 1000, pointsize=20)
...
# Salinity ONLY
png("Output/WGCNA/day2_larvae_hightemp/Day2_ClusterTree_Salinity.png", 1000, 1000, pointsize=20)
...
png("Output/WGCNA/day2_larvae_hightemp/Day2_ClusterTree_AragoniteSat.png", 1000, 1000, pointsize=20)
...
png("Output/WGCNA/day2_larvae_hightemp/Day2_ClusterTree_pCO2Salinity.png", 1000, 1000, pointsize=20)
...
# Group ONLY
png("Output/WGCNA/day2_larvae_hightemp/Day2_ClusterTree_Group.png", 1000, 1000, pointsize=20)
...
plotDendroAndColors(sampleTree2, traitColors_Group, # Plot the sample dendrogram and the colors underneath.
                    ...
                    main = "Sample dendrogram and trait heatmap (Group, Sal pCO2 AragSat)")
dev.off()
...
dim(dds.d2_vst) #  11 4858 - again double check you have the correct data...
...
png("Output/WGCNA/day2_larvae_hightemp/Day2_ScaleTopology_SoftThresh.png", 1000, 1000, pointsize=20)
        par(mfrow = c(1,2));
        cex1 = 0.9;
...
minModuleSize = 100; # set this for the subsequent call - WGCNA authors recommend diligence when calling module size to avoid too many/too few modules...
...
# 8 1023  708  468  461  415  365  356  276  223  212  174  169
...
# dynamicColors_sign
# black        blue       brown       green greenyellow        grey     magenta        pink      purple         red         tan   turquoise 
# 356         708         468         415         174           8         223         276         212         365         169        1023 
# yellow 
# 461 
# Plot the dendrogram and colors underneath
sizeGrWindow(8,6)
png("Output/WGCNA/day2_larvae_hightemp/day2_Dendrogram.png", 1000, 1000, pointsize=20)
...
# Plot the result ================================================================ #
sizeGrWindow(7, 6)
png("Output/WGCNA/day2_larvae_hightemp/Day2_ClusterEigengenes.png", 1000, 1000, pointsize=20)
plot(METree, main = "Clustering of module eigengenes - SIGNED (dissimilarity calc = MEDiss = 1-cor(MEs))",
     xlab = "", sub = "") +
     abline(h=0.525, col = "red") # add the line to the tree
dev.off()
...
MEDissThres = 0.525 # call the cut height
...
merge = mergeCloseModules(dds.d2_vst, dynamicColors_sign, cutHeight = MEDissThres, verbose = 3) # signed TOM 
...
table(merge$colors)
# black        blue       brown       green greenyellow        grey         red   turquoise 
# 356        1826         468         638         174           8         365        1023 


sizeGrWindow(18, 6)
png("Output/WGCNA/day2_larvae_hightemp/Day2_ClusterEigengenes_merged.png", 1000, 1000, pointsize=20)
...
sizeGrWindow(12, 9)
png("Output/WGCNA/day2_larvae_hightemp/Day2_ClusterDendrogram_merged.png", 1000, 1000, pointsize=20)
...
# save
save(MEs, dds.d2_vst, moduleLabels, moduleColors, file = "Output/WGCNA/day2_larvae_hightemp/day2-networkConstruction-stepByStep_merged.RData")
write.csv(MEs, file = "Output/WGCNA/day2_larvae_hightemp/d2.WGCNA_ModulEigengenes.csv")
...
# Construct numerical labels corresponding to the colors
colorOrder_merged = c("grey", standardColors(50));
moduleLabels_merged = match(moduleColors_merged, colorOrder_merged)-1;

# Save 
save(mergedMEs, dds.d2_vst, moduleLabels_merged, moduleColors_merged, file = "Output/WGCNA/day2_larvae_hightemp/day2-networkConstruction-stepByStep_merged.RData")
write.csv(mergedMEs, file = "Output/WGCNA/day2_larvae_hightemp/d2.WGCNA_ModulEigengenes_merged.csv")
...
# Recalculate MEs with color labels
MEs0           <-  read.csv("Output/WGCNA/day2_larvae_hightemp/d2.WGCNA_ModulEigengenes.csv") # read merged eigengene dataset
...
#=====================================================================================
#
# Module trait correlation - UNMERGED
#
#=====================================================================================
# ALL TRAIT DATA
...
png("Output/WGCNA/day2_larvae_hightemp/heatmaps/Day2_pCO2_heatmap.png", 500, 1000, pointsize=20)
...
pa                  = cluster::pam(d2.pCO2.COR, k = 5)
col_fun             = colorRamp2(c(-0.5, 0, 0.5), c("blue", "white", "red"))
pdf("Output/WGCNA/day2_larvae_hightemp/heatmaps/Day2_pCO2_heatmap.pdf", width=5, height=6)
Heatmap(moduleTraitCor_pCO2, 
        ...
        cell_fun = function(j, i, x, y, width, height, fill) {
          grid.text(sprintf("%.1f", d2.pCO2.text[i, j]), x, y, gp = gpar(fontsize = 10))
        })
dev.off()
...
par(mar = c(8, 9.5, 5, 3));
# Display the correlation values within a heatmap plot
png("Output/WGCNA/day2_larvae_hightemp/heatmaps/Day2_Salinity_heatmap.png", 500, 1000, pointsize=20)
...
col_fun             = colorRamp2(c(-0.5, 0, 0.5), c("blue", "white", "red"))
pdf("Output/WGCNA/day2_larvae_hightemp/heatmaps/Day2_Salinity_heatmap.pdf", width=5, height=6)
...
par(mar = c(8, 9.5, 5, 3));
# Display the correlation values within a heatmap plot
png("Output/WGCNA/day2_larvae_hightemp/heatmaps/Day2_AragoniteSat_heatmap.png", 500, 1000, pointsize=20)
...
col_fun             = colorRamp2(c(-0.5, 0, 0.5), c("blue", "white", "red"))
pdf("Output/WGCNA/day2_larvae_hightemp/heatmaps/Day2_AragoniteSat_heatmap.pdf", width=5, height=6)
...
par(mar = c(8, 9.5, 5, 3));
# Display the correlation values within a heatmap plot
png("Output/WGCNA/day2_larvae_hightemp/heatmaps/Day2_pCO2Salinity_heatmap.png", 500, 1000, pointsize=20)
...
pa                  = cluster::pam(d2.pCO2Salinity.COR, k = 5)
col_fun             = colorRamp2(c(-0.5, 0, 0.5), c("blue", "white", "red"))
pdf("Output/WGCNA/day2_larvae_hightemp/heatmaps/Day2_pCO2Salinity_heatmap.pdf", width=5, height=6)
...
par(mar = c(8, 9.5, 5, 3));
# Display the correlation values within a heatmap plot
png("Output/WGCNA/day2_larvae_hightemp/heatmaps/Day2_Group_heatmap.png", 1000, 1000, pointsize=20)
...
col_fun             = colorRamp2(c(-0.5, 0, 0.5), c("blue", "white", "red"))
pdf("Output/WGCNA/day2_larvae_hightemp/heatmaps/Day2_Group_heatmap.pdf", width=5, height=6)
...
MEsPlotting_melt      <- melt(MEsPlotting, id=c('Sample.Name','pCO2', 'Salinity', 'Aragonite_saturation','All_treatment', 'pCO2_Salinity', 'Temperature'))

#plot it - salinity and temperature
png("Output/WGCNA/day2_larvae_hightemp/Day2_ME_Boxplot_All.png", 600, 1000, pointsize=20)
...
png("Output/WGCNA/day2_larvae_hightemp/Day2_ME_Boxplot_Salinity_pCO2.png", 600, 1000, pointsize=20)
...
#plot it - salinity and pCO2
png("Output/WGCNA/day2_larvae_hightemp/Day2_ME_Boxplot_Aragonite_pCO2.png", 600, 1000, pointsize=20)
ggplot(MEsPlotting_melt, aes(x=Aragonite_saturation, y=value, fill = factor( pCO2), shape= pCO2)) +
  geom_boxplot(aes(middle = mean(value)), position=position_dodge(0.8), outlier.size = 0, alpha = 0.5) + 
  stat_summary(fun.y = mean, color = "black", position = position_dodge(0.75),
               geom = "point", shape = 19, size = 3,
               show.legend = FALSE) +
...
# HHM treatment group
HHM = as.data.frame(as.numeric(d2.Traits.Group$HHM )); # Define variable containing the desired column 
names(HHM) = "HHM"
HHM_geneTraitSignificance = as.data.frame(cor(dds.d2_vst, HHM, use = "p"));
...
names(HHM_geneTraitSignificance)
names(HHM_GSPvalue)
...
modOrder = order(-abs(cor(MEs, HHM, use = "p"))); # order by the strength of the correlation between module and trait values for each sample
...
write.csv(geneInfo_GROUPS, file = "Output/WGCNA/day2_larvae_hightemp/d2.WGCNA_ModulMembership.csv")
