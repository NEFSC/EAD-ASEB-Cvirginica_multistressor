...
## boxplots
```{r, bxplots all data facetted by age}
# ?scale_linetype
# plot all 
...
## regresions (exploratory for aragonite correlation)
```{r, regression with aragonite sat}
# ?scale_linetype
# plot all 
...
Master_Lengths_Treat_means <- Master_Lengths_Rep_means %>% 
                                  dplyr::select(-c(Replicate)) %>% 
                                  dplyr::group_by(Age, Temp, OA, Salinity, Aragonite_saturation) %>% 
                                  dplyr::summarise_each(funs(mean,sd,se=sd(.)/sqrt(n()))) 
...
# explore the mean SE and percent change with model results 
# main effects of pCO2 temperature and salinity 

Age1_raw_length %>% dplyr::select(OA, Temp, Salinity,length_um) %>% 
                      dplyr::group_by(OA, Temp, Salinity) %>% 
                      dplyr::summarise_each(funs(mean,sd,se=sd(.)/sqrt(n()), n = n()))
# N = 150 per treatment

#pCO2
age1_pCO2_meanSE <- Age1_raw_length %>% 
                      dplyr::select(OA,length_um) %>% 
                      dplyr::group_by(OA) %>% 
                      dplyr::summarise_each(funs(mean,sd,se=sd(.)/sqrt(n())))
age1_pCO2_meanSE
# High	70.19591	4.239491	0.1730765	
# Low	74.05942	3.682447	0.1503353
((74.05942-70.19591)/74.05942)*100 # 5.2 % larger under low pCO2

# Temperature
age1_Temp_meanSE <- Age1_raw_length %>% 
                      dplyr::select(Temp,length_um) %>% 
                      dplyr::group_by(Temp) %>% 
                      dplyr::summarise_each(funs(mean,sd,se=sd(.)/sqrt(n())))
age1_Temp_meanSE
# High	72.92781	4.429291	0.1808250	
# Low	71.32752	4.255381	0.1737252
((72.92781-71.32752)/72.92781)*100 # 2.2  % larger under high termperature

# Salinity
age1_Salinity_meanSE <- Age1_raw_length %>% 
                      dplyr::select(Salinity,length_um) %>% 
                      dplyr::group_by(Salinity) %>% 
                      dplyr::summarise_each(funs(mean,sd,se=sd(.)/sqrt(n()), n = n()))
age1_Salinity_meanSE
# High	74.16354	3.874498	0.1581757	
# Low	70.09179	3.961609	0.1617320	
((74.16354-70.09179)/74.16354)*100 # 5.5 % larger under high Salinity 

# pCO2 x Salinity interaction
age1_OASalinity_meanSE <- Age1_raw_length %>% 
                      dplyr::mutate(OA_Salinity = paste0(OA,"_x_",Salinity)) %>%                       
                      dplyr::select(OA_Salinity,length_um) %>% 
                      dplyr::group_by(OA_Salinity) %>% 
                      dplyr::summarise_each(funs(mean,sd,se=sd(.)/sqrt(n()), n = n()))
age1_OASalinity_meanSE
# High_x_High	72.68150	3.535329	0.2041123	300
# High_x_Low	67.71032	3.333242	0.1924448	300
# Low_x_High	75.64558	3.628713	0.2095038	300
# Low_x_Low	72.47325	2.991651	0.1727231	300
((75.64558-67.71032)/75.64558)*100 # 10.49005 % larger under Low_x_High than High_x_Low
...
# RUN ADONIS FOR EACH PAIR
D4_adonis_HH_LL   <- adonis2(D4_HH_LL_dist  ~ OA_Temp, data=D4_HH_LL_metadata, perms = permutations) # yup
...
D4_adonis_HL_LL   <- adonis2(D4_HL_LL_dist  ~ OA_Temp, data=D4_HL_LL_metadata, perms = permutations) # nope
...
# Summary stats for N per treatment
Age4_summary <- Age4_raw_length %>% dplyr::select(OA, Temp, Salinity,length_um) %>% 
                      dplyr::group_by(OA, Temp, Salinity) %>% 
                      dplyr::summarise_each(funs(mean,sd,se=sd(.)/sqrt(n()), n = n()))
mean(Age4_summary$n) # mean = 50.25
sd(Age4_summary$n)# SD = 11.49845

# explore the PERMANOVA responses - main effects of pCO2 and salinity followed by pCO2xtemperature interaction
...
((84.77301-76.70462)/84.77301)*100 # 9.51764 % larger under low pCO2
...
((83.46450-79.11545)/83.46450)*100 # 5.210658 % larger under high salinity
...
((88.33520-74.93013)/88.33520)*100 # 15.17523 % smaller under High_x_High relative to Low_x_High
((88.33520-78.16158)/88.33520)*100 # 11.51706 % smaller under High_x_Low relative to Low_x_High
```
...
Age8_summary <- Age8_raw_length %>% dplyr::select(OA, Temp, Salinity,length_um) %>% 
                      dplyr::group_by(OA, Temp, Salinity) %>% 
                      dplyr::summarise_each(funs(mean,sd,se=sd(.)/sqrt(n()), n = n()))
mean(Age8_summary$n) # mean = 210.375
sd(Age8_summary$n)# SD = 40.13882
...
((115.94102-97.11573)/115.94102)*100 # 16.23695 % larger under low pCO2
...
((114.35630-97.53579)/114.35630)*100 # 14.70886 % larger under high salinity
...
((124.69074-96.64262)/124.69074)*100 # 22.49415 % smaller under High_x_High relative to Low_x_High
((124.69074-97.54845)/124.69074)*100 # 21.76769 % smaller under High_x_Low relative to Low_x_High
```
...
Age11_summary <- Age11_raw_length %>% dplyr::select(OA, Temp, Salinity,length_um) %>% 
                      dplyr::group_by(OA, Temp, Salinity) %>% 
                      dplyr::summarise_each(funs(mean,sd,se=sd(.)/sqrt(n()), n = n()))
mean(Age11_summary$n) # mean = 55.875
sd(Age11_summary$n)# SD = 29.1422


# salinity main effect 
age11_Salinity_meanSE <- Age11_raw_length %>% 
                      dplyr::select(Salinity,length_um) %>% 
                      dplyr::group_by(Salinity) %>% 
                      dplyr::summarise_each(funs(mean,sd,se=sd(.)/sqrt(n())))
age11_Salinity_meanSE
# High	144.4325	50.91659	2.910708	
# Low	118.0298	30.66179	2.582189
((144.4325-118.0298)/144.4325)*100 # 18.2803 % larger under high salinity
...
((160.0100-107.1093)/160.0100)*100 # 33.06087 % larger under high temperature
...
((170.8969-105.5895)/170.8969)*100 # 38.2145 % smaller under Low_x_High relative to High_x_High
((170.8969-128.5592)/170.8969)*100 # 24.77383 % smaller under High_x_Low relative to High_x_High
```
...
Age15_summary <- Age15_raw_length %>% dplyr::select(OA, Temp, Salinity,length_um) %>% 
                      dplyr::group_by(OA, Temp, Salinity) %>% 
                      dplyr::summarise_each(funs(mean,sd,se=sd(.)/sqrt(n()), n = n()))
mean(Age15_summary$n) # mean = 47.14286
sd(Age15_summary$n)# SD = 25.91929
...
((221.8048-189.7829)/221.8048)*100 # 14.43697 % larger under high salinity
...
((255.0615-125.3223)/255.0615)*100 # 50.86585 % larger under high temperature
...
((270.1614-122.4419)/270.1614)*100 # 54.67824 % smaller under Low_x_High relative to High_x_High
((270.1614-222.2357)/270.1614)*100 # 17.73965 % smaller under High_x_Low relative to High_x_High
...
GR_means <- as.data.frame(Length_means %>% 
  ...
  dplyr::mutate(Growth_rate_umday_d4_d15 = (`15`- `4`) / 9) %>% 
  dplyr::select(!c(`1`,`4`,`15`))) # note 1 missing value for OA High Temp Low Salinity Low

# once value has no length values on day 15 - calculate for day 11 endpoint
getNA <- as.data.frame(Length_means %>% 
  filter(Age %in% c(1,4,11)) %>% 
  dplyr::select(c(Age, OA, Salinity, Temp, mean_length)) %>% 
  dplyr::group_by(OA, Salinity, Temp)  %>% 
  tidyr::spread(Age, mean_length) %>%  # new columns titled 1 and 15 for he Age and with values for the 
  # length data grouped by treatment 
  dplyr::mutate(Growth_rate_umday = (`11` - `1`) / 13) %>% 
  dplyr::mutate(Growth_rate_umday_d4_d15 = (`11` - `4`) / 7) %>% 
  dplyr::select(!c(`1`,`4`,`11`))) # 2.612424 for OA High Temp Low Salinity Low, insert into the other table

GR_means[4,4] = 2.612424
GR_means[4,5] = 4.030921

GR_means$Growth_rate_umday        <- as.numeric(GR_means$Growth_rate_umday)
GR_means$Growth_rate_umday_d4_d15 <- as.numeric(GR_means$Growth_rate_umday_d4_d15) # with the exception of one value with d 11 enpoint!
...
Heatplot_larvae_4_to_15 <- Length_means_MASTER_larvae %>% 
                      ...
                      dplyr::mutate(OA_Sal = 
                       ...
                       ggplot(aes(x = Age,
                                  y = mean_length)) + 
                       geom_rect(aes(fill = Growth_rate_umday_d4_d15), 
                                   xmin = -Inf, 
                                   xmax = Inf, 
                                   ymin = -Inf, 
                                   ymax = Inf, 
                      ...
                      geom_text(size = 2, 
                                   aes(x = 12, 
                                       y = 0.8, 
                                       label = paste0(round(Growth_rate_umday_d4_d15,2),"um day-1"))) +
                      scale_fill_gradient(low = "orange", 
                                           high = "forestgreen", 
                                           aesthetics = "fill")
...
setwd("C:/Users/samjg/Documents/Github_repositories/Cvirginica_multistressor/RAnalysis")
pdf("Output/Length/plots/Length_day4_day15.pdf", width=11, height=5)
print(Heatplot_larvae_4_to_15)
dev.off()
