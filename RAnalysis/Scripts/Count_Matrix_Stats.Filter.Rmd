...
# FORMT AND EXPORT READ COUNT MATRIX (sum lanes of common sample ID):
...
sum_raw_counts <- subset(sum_raw_counts, select = -c(transcript_sample_IDs) ) # omit the merged column 
...
write.csv(sum_raw_counts_matrix,paste(path,"Sum_raw_count_matrix.csv", sep =''))
...
nrow(raw_counts_mtx.ALL) # 67334 total unique transcript IDs
View(raw_counts_mtx.ALL[2]) # tRNA == 'rna-NC_....'  'rna-Trna..'    pseudogenes == 'gene-LOC111.....'  mitochondrion  == 'gene-'      tRNA == 'rna-Trna'
...
length(unique(SeqIDs$GeneID)) # 22262 unique gene IDs (LOC....)
length(unique(SeqIDs$TranscriptID)) # 66625 unique transcript IDs (X...)
write.csv(SeqIDs,"C:/Users/samuel.gurr/Documents/Github_repositories/Cvriginica_multistressor/RAnalysis/Data/TagSeq/Seq_details/seq_id_master.csv") # Write new raw read file



# Reference; sample IDs and corresponding treatments 
smpl_treatment_ref  <- read.csv(file="../Data/TagSeq/Seq_details/Sample_Key.csv", sep=',', header=TRUE) # reference to



ncol(raw_counts_mtx) -1 # 36 total columns without the transcript ID

# how many genes are here?
Cvirginica_v3.0_rna.fna <- read.delim2(file="C:/Users/samuel.gurr/Documents/Genomes/Crassostrea_virginica/GCF_002022765.2_C_virginica-3.0_rna.fna", header=F)
head(Cvirginica_v3.0_rna.fna) %>% dplyr::filter(grepl(">"), )
