...
for(i in 1:nrow(modcolor)) {
  ...
  meanEXp_Mod <- merged_Expdata_Mod %>% 
    select(c('Sample.Name','rlog_Expression', 'Salinity', 'pCO2')) %>% 
    group_by(Sample.Name, Salinity, pCO2) %>%
  ...
  single.factor.plot <-  ggarrange(Salinity.rlog.Mod,  pCO2.rlog.Mod,      
                                   plotlist = NULL,
                                   ncol = 3,
                                   nrow = 1,
                                   labels = NULL)
