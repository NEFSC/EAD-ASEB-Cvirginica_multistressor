...
Cvirginica_annot_reference  <- read.csv(file="../Data/TagSeq/Seq_details/seq_id_master.csv", sep=',', header=TRUE) %>% 
                                ...
                                dplyr::mutate(Function = gsub("\\s\\(LOC.*|\\sLOC111.*", "", perl=TRUE, Function))
nrow(Cvirginica_annot_reference) #66625 including all genes
nrow(Cvirginica_annot_reference %>% dplyr::filter(grepl('uncharacterized', Function))) #27164 EXCLUDING those labeled  as 'uncharacterized'

# GO terms: upload the Crassostrea gigas (Pacific oyster) data downloaded from Uniprot (contains GO term annotation!)
Cgigas_GOterms  <- read.csv(file="../Data/TagSeq/Seq_details/uniprot_Crassostrea_gigas.csv", sep=',', header=TRUE) %>% 
                      dplyr::select(c('','','','','Protein.names','Gene.ontology.IDs'))
  
  
  read_excel("../Data/TagSeq/Seq_details/uniprot_Crassostrea_gigas") 
read.table(readLines("../Data/TagSeq/Seq_details/uniprot_Crassostrea_gigas.xlsx", warn = FALSE), sep = "\t", header=TRUE)
...
Cgigas_KEGGIDs <-as.data.frame(read.delim2(file = "C:/Users/samjg/Documents/Bioinformatics/refs/Cgigas/T03920_(2021_06_23 21_09_09 UTC).nuc", header=FALSE)) %>% 
                      filter(grepl(">crg:", V1)) %>% 
                      dplyr::mutate(KEGG_ID = gsub("\\s.*|>", "",V1)) %>% 
...
ref           <- Cvirginica_annot_reference[,c(2:3)] %>%
                 dplyr::mutate(Function = gsub("\\s\\(LOC.*|\\sLOC111.*", "", perl=TRUE, Function))

nrow(ref)

ref_withCgigasKEGG <- merge(ref, Cgigas_KEGGIDs)
...
DE_Master <- rbind(DE_d2.Temperaturey_SHORT, DE_d2.Salinity_SHORT, DE_d2.OA_SHORT, 
                   DE_d18.Salinity_SHORT, DE_d18.OA_SHORT)

DE_Master_with_annotation <- merge(DE_Master, ref, by="TranscriptID")
...
All.Salinity.UPREG        <- rbind(DE_d2.Salinity.UPREG,DE_d18.Salinity.UPREG) %>% add_count(TranscriptID)# UPREG call
Constant_UPREG_Salinity   <- unique(All.Salinity.UPREG %>% dplyr::filter(n %in% 2) %>%  dplyr::select(c('Function','TranscriptID')) %>% dplyr::filter(!Function %in% " uncharacterized")  )

All.Salinity.DWNREG       <- rbind(DE_d2.Salinity.DWNREG,DE_d18.Salinity.DWNREG) %>% add_count(TranscriptID)# UPREG call
Constant_DWNREG_Salinity  <- unique(All.Salinity.DWNREG %>% dplyr::filter(n %in% 2) %>%  dplyr::select(c('Function','TranscriptID')) %>% dplyr::filter(!Function %in% " uncharacterized")  )


All.OA.UPREG              <- rbind(DE_d2.OA.UPREG,DE_d18.OA.UPREG) %>% add_count(TranscriptID)# UPREG call
Constant_UPREG_OA         <- unique(All.OA.UPREG %>% dplyr::filter(n %in% 2) %>%  dplyr::select(c('Function','TranscriptID')) %>% dplyr::filter(!Function %in% " uncharacterized")  )

All.OA.DWNREG             <- rbind(DE_d2.OA.DWNREG,DE_d18.OA.DWNREG) %>% add_count(TranscriptID)# UPREG call
Constant_DWNREG_OA        <- unique(All.OA.DWNREG %>% dplyr::filter(n %in% 2) %>%  dplyr::select(c('Function','TranscriptID')) %>% dplyr::filter(!Function %in% " uncharacterized")  )
