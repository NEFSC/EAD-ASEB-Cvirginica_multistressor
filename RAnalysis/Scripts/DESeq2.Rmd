...
library(ggplot2)
library(cowplot)
library(pheatmap) # BiocManager::install
library(gplots)
library(RColorBrewer)
library(pcaExplorer) # BiocManager::install
# install.packages('')
...
d18.metadata     <- d18.exp_data %>% 
                    dplyr::select(c('SapleName_readmatrix', 
                                    'Temperature', 'OA', 'Salinity', 'Aragonite_saturation')) %>% # coondense dataset to build target matrix
                    dplyr::rename("Sample.Name" = "SapleName_readmatrix") %>% 
                    dplyr::mutate(All_Treatment = paste(Temperature, OA, Salinity, sep = '.'))
...
dds.d18.arag.sat <- DESeqDataSetFromMatrix(countData = d18.counts_matrix,
                                        colData = d18.metadata.mtx,
                                        design = ~ Aragonite_saturation)
...
dds.d18.arag.sat <- DESeq(dds.d18.arag.sat) # arag sat model
...
## pca ggpubr for reviewer reponse
...
ggplot(pcaData_d2, aes(x = PC1, y = PC2,  alpha = OA, shape = Salinity, color = Temperature, label=name)) +
  ...
  theme_classic() +
  theme(text = element_text(size=15)) +
  ggtitle("PCA: Day2 (rlog) all experimental treatments") +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  coord_fixed(),
...
rlog.d18.arag.sat <- rlogTransformation(dds.d18.arag.sat)
...
ggplot(pcaData_d18, aes(x = PC1, y = PC2,  alpha = OA, colour = group, shape = Salinity, label=name)) +
  geom_point(size =6) +
  scale_color_manual(values = c("#D55E00", "#D55E00", "#D55E00","#D55E00")) +
  scale_alpha_manual(values = c(0.5, 1)) +
  scale_shape_manual(values = c(4, 19, 17)) +
  geom_text(aes(label=name),hjust=0.2, vjust=1.4, size=5) +
  theme_classic() +
...
# PCA plot rlog.d18.arag.sat ::::::::::::::::::::::::::::::::::::::::::
pcaData_d18.arag.sat <- plotPCA(rlog.d18.arag.sat, intgroup = ("Aragonite_saturation"), returnData = TRUE)
percentVar.arag.sat <- round(100 * attr(pcaData_d18.arag.sat, "percentVar"))
png("../Output/DESeq2/Day18_spat/plots/Day18.rlog_PCA_aragonite_sat.png", 1000, 1000, pointsize=20)
ggplot(pcaData_d18.arag.sat, aes(x = PC1, y = PC2, color = Aragonite_saturation, label=name)) +
  geom_point(size =6) +
  scale_color_manual(values = c("#D55E00", "#e69F00", "#56B4E9", "#0072B2")) +
  geom_text(aes(label=name),hjust=0.2, vjust=1.4, size=5) +
  theme_classic() +
  theme(text = element_text(size=15)) +
  ggtitle("PCA: Day 22 (rlog) aragonite saturation") +
  xlab(paste0("PC1: ", percentVar.arag.sat[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar.arag.sat[2], "% variance")) +
  coord_fixed()
dev.off()
...
## plot together for peer review response
ggpubr::ggarrange(
    ...
    ggplot(pcaData_d18, aes(x = PC1, y = PC2,  alpha = OA, colour = group, shape = Salinity, label=name)) +
      ...
      ggtitle("PCA: Day 22 (rlog) experimental treatments") +
      xlab(paste0("PC1: ", percentVar[1], "% variance")) +
      ylab(paste0("PC2: ", percentVar[2], "% variance")) +
      coord_fixed(),
